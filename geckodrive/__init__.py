...
def estopdrive(S=None,port=None):
    """
...
def movedrive(S, axis:str, dist_inch, steps_per_inch:int,
                                                port=None,verbose:bool=False):
    if not S or not S.isOpen():
        S=connectdrive(port)
        configdrive(S,port)
...
def distinch2step(dist_inch, steps_per_inch:int=10000, verbose:bool=False) -> int:
    """
...
def distcm2step(dist_cm, steps_per_inch:int=10000, verbose:bool=False) -> int:
    """
    verify steps per inch with your drive!!
